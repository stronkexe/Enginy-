...
impl<'a> HtmlParser<'a> {
        ...
        while self.chars.peek().is_some() {
            ...
            if self.chars.peek().map_or(false, |c| *c == '<') {
                ...
                if self.chars.peek().map_or(false, |c| *c == '/') {
                    ...
                    self.chars.next();
                    node.push(self.parse_comment_node());
                }
                else {
                ...
                nodes.push(self.parse_next_node());
            }
        }
        nodes
    }
    ...
    fn parse_node(&mut self) -> Node {
        ...
        let elem = ElementData::new(tag_name, attributes);
    ...
    fn parse_text_node(&mut self) -> Node {
        ...
        while self.chars.peek().map_or(false, |c| *c != '<') {
            ...
            text_content.push_text(&text_part)
        }
...
fn is_valid_attr_name(c: char) -> bool {
    !is_excluded_name(c) && !is_controle(c)
}

fn is_controle(ch: char) -> bool {
    match ch {
        '\u{007F}' => true,
